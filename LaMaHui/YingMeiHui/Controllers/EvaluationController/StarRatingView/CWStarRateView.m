...
- (void)layoutSubviews {
    ...
    [UIView animateWithDuration:animationTimeInterval animations:^{
        // 保留小数后俩位
        CGFloat sp = weakSelf.scorePercent;
        //        CGFloat sp =( (float)( (int)( (weakSelf.scorePercent+0.005)*100 ) ) )/100;
        if (sp <= 0) {
            sp = 0;
        }else if (sp > 0.05 && sp <= 0.1) {
            sp = 0.1;
        }else if(sp > 0.1 && sp <= 0.2) {
            sp = 0.2;
        }else if(sp > 0.2 && sp <= 0.3) {
            sp = 0.3;
        }else if(sp > 0.3 && sp <= 0.4) {
            sp = 0.4;
        }else if(sp > 0.4 && sp <= 0.5) {
            sp = 0.5;
        }else if(sp > 0.5 && sp <= 0.6) {
            sp = 0.6;
        }else if(sp > 0.6 && sp <= 0.7) {
            sp = 0.7;
        }else if(sp > 0.7 && sp <= 0.8) {
            sp = 0.8;
        ...
        weakSelf.foregroundStarView.frame = CGRectMake(0, 0, weakSelf.bounds.size.width * sp, weakSelf.bounds.size.height);
    }];
}
