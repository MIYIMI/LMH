...
@property (nonatomic, assign) NSInteger previousOrientation;
...
//    return [_messageTitle ?: [self localizedStringForKey:iRateMessageTitleKey withDefault:@"Rate %@"] stringByReplacingOccurrencesOfString:@"%@" withString:self.applicationName];
    ...
    NSString *str = @"娘娘喜欢我们的APP吗，求赏赐好评！";
    return str;
}
...
//    return _cancelButtonLabel ?: [self localizedStringForKey:iRateCancelButtonKey withDefault:@"No, Thanks"];
    ...
    return _cancelButtonLabel ?: [self localizedStringForKey:iRateCancelButtonKey withDefault:@"退下"];
...
//    return _rateButtonLabel ?: [self localizedStringForKey:iRateRateButtonKey withDefault:@"Rate It Now"];
    ...
    return _rateButtonLabel ?: [self localizedStringForKey:iRateRateButtonKey withDefault:@" 乖，给本宫赏 ！"];
...
//    return _remindButtonLabel ?: [self localizedStringForKey:iRateRemindButtonKey withDefault:@"Remind Me Later"];
    ...
    return _remindButtonLabel ?: [self localizedStringForKey:iRateRemindButtonKey withDefault:@"稀烂，赐一丈红"];
...
- (CGFloat)usesPerWeek
{
    return (CGFloat)self.usesCount / ([[NSDate date] timeIntervalSinceDate:self.firstUsed] / SECONDS_IN_A_WEEK);
}
...
- (void)promptForRating
{
    if (!self.visibleAlert)
    {
        ...
        [self performSelector:@selector(showAlert) withObject:nil afterDelay:30];
#else
...
//        [self.delegate iRateUserDidRequestReminderToRateApp];
        ...
        kata_FeedbackManageViewController *feedController = [[kata_FeedbackManageViewController alloc] init];
        self.navigationController = [[(kata_AppDelegate *)[[UIApplication sharedApplication] delegate] rootVC] navigationController];
