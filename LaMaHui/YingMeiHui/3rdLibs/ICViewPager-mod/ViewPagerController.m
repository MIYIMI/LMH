...
#define kDefaultContentViewBackgroundColor [UIColor colorWithRed:0.91 green:0.91 blue:0.91 alpha:1]
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.selected) {
        ...
        [bezierPath moveToPoint:CGPointMake(rect.size.width / 2.0 - 22, rect.size.height-1)];
        [bezierPath addLineToPoint:CGPointMake(rect.size.width / 2.0 + 22, rect.size.height-1)];
        [bezierPath addLineToPoint:CGPointMake(rect.size.width / 2.0 + 22, rect.size.height - 1)];
        [bezierPath addLineToPoint:CGPointMake(rect.size.width / 2.0 - 22, rect.size.height - 1)];
...
- (IBAction)handleTapGesture:(id)sender {
    ...
    [self selectedTabIndex:index]; //add by kevin on 2014.08.26 pm 14:30
    return;
...
-(void)selectedTabIndex:(NSInteger)_index{  //add by kevin on 2014.08.26 pm 14:30
    
    __block NSUInteger index = _index;
    // Get the desired viewController
    UIViewController *viewController = [self viewControllerAtIndex:index];
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    for (id view in activeTabView.subviews) {
        if ([view isKindOfClass:[UILabel class]]) {
            UILabel *lbl = (UILabel *)view;
            lbl.font = [UIFont systemFontOfSize:14.0];
            lbl.textColor = LMH_COLOR_BLACK;
        }
    }
...
- (void)defaultSettings {
    ...
    [((UIScrollView*)[_pageViewController.view.subviews objectAtIndex:0]) setDirectionalLockEnabled:YES];
